# From "Balanced Datasets are Not Enough" repo: https://github.com/uvavision/Balanced-Datasets-Are-Not-Enough/blob/master/object_multilabel/data_loader.py
...
def get_data_loader_coco(args):
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size = args.train_bs,
            shuffle = True, num_workers = 6, pin_memory = True)

    test_loader = torch.utils.data.DataLoader(test_data, batch_size = args.test_bs,
...
class CocoObjectGender(data.Dataset):
    def __init__(self, args, annotation_dir, image_dir, split = 'train', transform = None, \
        ...
        if args.balanced:
            balanced_subset = pickle.load(open(os.path.join(annotation_dir, "{}_ratio_{}.ids".format(split, \
                args.ratio)), 'rb'))
