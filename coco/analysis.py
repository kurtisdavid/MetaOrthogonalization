...
def compute_tcavs(dataloader,
    ...
    model.eval()
    collect = np.zeros(
        len(dataloader.dataset) * objects
    )
    labels = np.zeros(
        len(dataloader.dataset) * objects
    ).astype(np.int)
    ...
    for i,(X,y,gender) in enumerate(dataloader):
        ...
        if object_detect:
            assert objects == y.shape[1]
            with torch.no_grad():
                for j in range(objects):
                    has_object = (y[:,j] == 1)
                    # create a vector of fake labels, only keeping the ones we want
                    tmp_y = torch.zeros(y.shape[0]) + j
                    tmp_y = tmp_y[has_object]
                    # extract the object specific logit gradient and filter
                    curr_S = S[:, j]
                    curr_S = curr_S[has_object]
                    if curr_S.shape[0] > 0:
                        collect[last:last+curr_S.shape[0]] = curr_S.data.cpu().numpy().squeeze()
                        labels[last:last+curr_S.shape[0]]  = tmp_y.data.cpu().numpy().squeeze()
                        last += curr_S.shape[0]
            assert (old_last + y.sum().item()) == last
        else:
            S = torch.gather(
            ...
            last += S.shape[0]
    if last < collect.shape[0]:
        print("LOOK HOW MANY:", last)
